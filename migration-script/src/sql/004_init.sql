...
BEGIN
    ...
    DELETE FROM public.post_assets WHERE post_id = p_post_id;

    -- Delete associated reactions
    DELETE FROM public.post_reaction WHERE post_id = p_post_id;

    -- Delete associated comments and their reports
    DELETE FROM public.post_comment_report 
    WHERE post_comment_id IN (
        SELECT id FROM public.post_comment WHERE post_id = p_post_id
    );

    DELETE FROM public.post_comment WHERE post_id = p_post_id;

    -- Delete post reports
    DELETE FROM public.post_report WHERE post_id = p_post_id;

    -- Finally delete the post
    DELETE FROM public.post WHERE id = p_post_id AND user_id = p_user_id;
...
BEGIN
    ...
    WHERE p.id = p_post_id AND p.is_blocked = false AND u.is_blocked = false AND u.is_active = true
    GROUP BY p.id, p.content, p.is_blocked, p.created_at, p.updated_at, u.id, u.firstname, u.lastname, u.username;
END;
$$;
...
BEGIN
    ...
    LEFT JOIN public."user" u ON p.user_id = u.id
    LEFT JOIN public.post_assets pa ON p.id = pa.post_id AND pa.is_active = true
    WHERE p.is_blocked = false AND u.is_blocked = false AND u.is_active = true
...
-- Created On    : 18/02/2026
-- ============================================================================
CREATE OR REPLACE FUNCTION fn_renumber_post_asset_positions(
    p_post_id UUID,
    p_user_id UUID
)
RETURNS INTEGER
...
-- Created On    : 18/02/2026
-- ============================================================================
CREATE OR REPLACE FUNCTION fn_get_max_post_asset_position(
    p_post_id UUID
)
RETURNS INTEGER
LANGUAGE plpgsql
...
-- Created On    : 18/02/2026
-- ============================================================================
CREATE OR REPLACE FUNCTION fn_ensure_image_asset_type()
RETURNS UUID
LANGUAGE plpgsql
AS $$
DECLARE
...
-- Created On    : 18/02/2026
-- ============================================================================
CREATE OR REPLACE FUNCTION fn_insert_post_assets(
    p_assets JSONB[]
)
RETURNS INTEGER
LANGUAGE plpgsql
...
BEGIN
    -- Ensure image asset type exists
...
-- Created On    : 18/02/2026
-- ============================================================================
CREATE OR REPLACE FUNCTION fn_get_active_post_assets_count(
    p_post_id UUID
)
RETURNS INTEGER
LANGUAGE plpgsql
