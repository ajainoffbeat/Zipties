...
CREATE OR REPLACE FUNCTION public.fn_search_posts(
...
SELECT 
    ...
    COUNT(DISTINCT CASE 
        WHEN prt.name = 'like' AND pr.is_active = B'1'
        THEN pr.id 
    END)::int AS like_count,
    ...
    EXISTS (
        SELECT 1
        FROM public.post_reaction pr_inner
        LEFT JOIN public.post_reaction_type prt_inner
            ON pr_inner.post_reaction_type_id = prt_inner.id
        WHERE pr_inner.post_id = p.id
          AND pr_inner.user_id = p_user_id
          AND prt_inner.name = 'like'
...
LEFT JOIN public.post_assets pa
    ON p.id = pa.post_id
    AND pa.is_active = true

LEFT JOIN public.post_reaction pr
    ON p.id = pr.post_id
    AND pr.is_active = B'1'

LEFT JOIN public.post_reaction_type prt
    ON pr.post_reaction_type_id = prt.id
    AND prt.name = 'like'
    AND prt.is_active = B'1'
...
WHERE 
    ...
    AND NOT EXISTS (
        ...
        WHERE ur.blocked_user_id = p.user_id
    ...
    AND NOT EXISTS (
        SELECT 1
        FROM public.user_report ur
        WHERE ur.blocked_user_id = p_user_id
...
DROP FUNCTION IF EXISTS public.fn_get_user_profile(uuid, uuid);
 
CREATE OR REPLACE FUNCTION public.fn_get_user_profile(
	p_current_user_id uuid,
	p_user_id uuid)
    RETURNS TABLE(id uuid, first_name character varying, last_name character varying, username character varying, email character varying, bio character varying, profile_image_url text, city_id uuid, city_name character varying, interests text, tags text, joined_date timestamp without time zone, isblocked boolean, followers_count integer, following_count integer, is_following boolean) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE PARALLEL UNSAFE
    ROWS 1000
...
BEGIN
    ...
    SELECT 
        ...
        EXISTS (
            ...
            FROM public.user_report ur 
            WHERE 
                (ur.user_id = p_current_user_id AND ur.blocked_user_id = p_user_id)
                OR 
                (ur.user_id = p_user_id AND ur.blocked_user_id = p_current_user_id)
        ),
        COALESCE(fc.follower_count, 0),
        COALESCE(foc.following_count, 0),
        public.fn_is_following(p_current_user_id, p_user_id)
    FROM public."user" u
